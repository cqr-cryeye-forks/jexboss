...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--auto-exploit", "-A",
                        ...
                        action='store_true')
    parser.add_argument("--disable-check-updates", "-D",
                        help="Disable update checks by webshell and client",
                        action='store_true')
    parser.add_argument('-mode', choices=['standalone', 'auto-scan', 'file-scan'],
                        default='standalone', help="Operation mode (DEFAULT: standalone)")
    ...
    parser.add_argument("--jenkins", help="Check only for Jenkins CLI vector.", action='store_true')
    parser.add_argument("--struts2", help="Check only for Struts2 Jakarta Multipart parser.",
                        action='store_true')
    parser.add_argument("--jmxtomcat",
    ...
    group_standalone.add_argument("-host", "-u", metavar='URL',
                                  help="Target URL (eg. -u http://host:8080)",
                                  required=True)

    # auto-scan mode
    group_auto_scan.add_argument("-network", type=network_args,
                                 default='192.168.0.0/24',
                                 help="Network in CIDR format for auto-scan")
    group_auto_scan.add_argument("-ports", default='8080,80',
                                 help="Comma-separated ports for auto-scan")
    group_auto_scan.add_argument("-results", metavar='FILENAME',
                                 default='jexboss_auto_scan_results.log',
                                 help="File to store auto-scan results")

    # file-scan mode
    group_file_scan.add_argument("-file", metavar='FILENAME_HOSTS',
                                 help="File with host list for file-scan")
    group_file_scan.add_argument("-out", metavar='FILENAME_RESULTS',
